---
title: 'Lecture 2.2: Data Analysis Tools'
...
The dplyr functions introduced last week are useful tools for understanding and managing your dataset: `filter()`, `arrange()`, `select()`, `mutate()` and `rename()`. They help us understand what columns of data look like... and by extension may suggest certain data cleaning tasks to make the information we want to work with clear, consistent, and ready for statistical analysis.
...
There are a number of functions available to recode values, such as `case_when()`, `if_else()`, and `recode()`. These functions can be used as helper functions within `mutate()` to replace certain values in a column of data with other values. Here we'll put `case_when()` to use in order to rename certain countries according to their most recent, UN-recognized names. We'll create a new variable called `country_clean` and assign it to a new data frame `gap2`.

```{r}
gap2 <- gapminder %>% 
  mutate(country_clean = case_when(country == 'Czech Republic' ~ 'Czechia',
                                   country == 'Swaziland' ~ 'Eswatini',
                                   country == "West Bank and Gaza" ~ "Palestine"))
```

Recall that country is stored as a factor. There are a number of functions designed specifically for working with factors and factor levels.
...
`if_else()` is a useful dplyr function (in the tidyverse) for manipulating data in columns of data based on conditional statements. To get a sense of how `if_else()` works, let's create a binary (logical) variable called `cont_asia` equal to `TRUE` if `continent == "Asia"` and `FALSE` for other continents.
...
Also note that base R includes its own `ifelse()` function that works a bit differently than the dplyr `if_else()` function. The dplyr version is a more rigid but safer option when it comes to working with different data types.
...
summary(gap3$continent)
summary(as.character(gap3$cont_asia))
summary(gap3$cont_asia)
```
...
```{r message = FALSE, warning = FALSE}
# joint distribution
table(gap3$continent, gap3$cont_asia)

# distribution of continent conditional on being in Asia (admittedly not very interesting)
prop.table(table(gap3$continent, gap3$cont_asia) , 2)  %>% round(2)
```
...
gapminder %>%
  ...
  group_by(year) %>%
  filter(min_rank(desc(lifeExp)) == 1 | min_rank(lifeExp) == 1) %>% # min_rank is a dplyr ranking function
