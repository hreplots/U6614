################################################################################
##
## [ PROJ ] DSP IA7514 Assignment 3: Subway Fare Evasion Arrests: Exploring Racial Disparities
## [ FILE ] Lecture3new-FULL.r 
## [ AUTH ] < YOUR NAME >
## [ INIT ] < Feb 2, 2026 >
##
################################################################################
...
# 3a.
  ...
  # alternative quadratic syntax
  m_3a_q <- lm_robust(arrperswipe ~ poly(povrt_all_2016, 2, raw = TRUE),
                      data = stations,
                      weight = swipes2016) #include quadratic term
  summary(m_3a_q) 
...
# 4a.
  ...
  # BONUS: how we can turn into a traditional 2x2 table?
  stations %>% 
    group_by(nblack, highpov) %>% 
    summarise(mean = round(weighted.mean(arrperswipe, swipes2016), 2)) %>% 
...
# 5a.
  ...
  stations <- stations %>% 
    inner_join(st_crime, by = c("st_id" = "st_id")) %>% 
    group_by(st_id, mta_name) %>% 
    arrange(desc(crimes)) 
  
  str(stations$crimes) # inspection only, do not include in submission
