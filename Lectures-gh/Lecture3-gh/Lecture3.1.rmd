---
title: 'Lecture 3.1: Data Analysis Tools Part 2'
subtitle: "DSPC7514 Data Analysis for Policy Research"
...
Of course we can still temporarily treat `country` as a factor to count the number of distinct countries observed in the gapminder data for 2002 and 2007.
...
To learn different `dplyr` join functions, we'll be working with a .csv file including annual country-level observations from the [World Bank](https://data.worldbank.org/indicator/SH.XPD.CHEX.PC.CD) on health expenditures per capita (from 2002-2017). Let's take a look.
...
`inner_join(df1, df2, by = col_to_join_by)` returns all rows in `df1` with values for `col_to_join_by` that match values in `df2`, and including all columns from both `df1` and `df2`. In this case we want to match on two columns that appear in both data frames, `country` and `year`. If there are multiple matches between `df1` and `df2`, all combinations of the matches are returned, which may not be what you want. This is considered a mutating join because we're adding columns to an existing data frame, in a similar fashion as the `mutate` function.

```{r}
# inner join by country-year with gap on the left, healthexp on the right
gap_joined_inner <-  
  gap %>% inner_join(healthexp, by = c("country", "year")) %>% 
...
In the previous code chunk, we *coerced* `country` in the joined data frame from a character back to a factor, and we can confirm that 132 factor levels for `country` (e.g. countries) from `gap` were retained in the inner join. This means 12 countries in the gapminder data were not matched to the health expenditure data. 
...
`full_join(df1, df2, by = col_to_join_by)` return all rows and all columns from both `df1` and `df2`. For rows in each data frame with no matching values for `col_to_join_by` in the other, the rows are retained with `NA` values assigned to elements of missing column data. If there are multiple matches between `df1` and `df2`, all combination of the matches are returned, which may not be what you want. This is a mutating join where all elements of both data frames are retained in the new, joined data frame.
...
Notice from the dimensions of the resulting data frame (4,244 x 8) that this approach retains all year-country gapminder observations, including those with no health expenditure data. It also retains observations for countries with health expenditure data that aren't included in the gapminder data (e.g. Azerbaijan). We can confirm that there are now 274 factor levels for `country`, i.e. 274 countries that appear in one data frame or the other. This is useful to know, but if we're interested in exploring the relationship between life expectancy and health expenditures, we probably want to analyze a data frame with only countries appear in 
...
Here we can see all `r nlevels(gap_joined_inner$country)` countries are retained from the gapminder data, even though we know that 12 countries from the gapminder data were not matched to countries in the health expenditure data.


## Filtering joins

Filtering joins keep cases from the left-hand data. 
...
This shows why `anti_join()` is a great tool for inspecting matches when we call join functions. We've now identified 10 countries from the left-hand data frame that don't appear in the right-hand data frame, and we should investigate further to see if this is just due to alternative country spellings, or countries in gapminder that are in fact missing from the health expenditure data.
...
This is the same functionality we introduced in Lecture3.1, but here we'll be using it to generate a new data frame for ongoing analysis not just summary statistics by group.
...
gap_global <- gapminder %>% 
  ...
  summarize(population = sum(pop), 
            gdp = sum(gdp_tot)) %>% # compute global pop and GDP
...
gap_z <- gapminder %>% 
  group_by(year) %>% 
  mutate(mean_life = mean(lifeExp), # mean lifeExp across countries by year
         sd_life = sd(lifeExp),      # s.d. of lifeExp across countries by year
         life_z = (lifeExp - mean_life) / sd_life) %>% # normalize every lifeExp obs to a z-score (within-year) 
  arrange(desc(life_z))

# look at most extreme country-year life expectancy obs after standardization
gap_z %>% head(n = 5)
gap_z %>% tail(n = 5)
```
...
In any event, getting a handle on which values should be treated as missing and why that is should be on the top of your to-do list. In some instances you may want to keep all observations and exclude `NA`'s as needed when making certain calculations. In other instances, you may want to drop incomplete rows from your analysis so that your sample remains the same throughout your analysis.
...
healthexp %>% filter(year == 2002, 
                     is.na(healthexp) == TRUE)
```
...
```
Note that `NA` observations like Aruba have been dropped from `healthexp_2002` (but we can still access these observations in `healthexp` if need be).
