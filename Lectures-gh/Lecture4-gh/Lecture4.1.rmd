---
title: 'Lecture 4.1: Exploratory Data Analysis and Weighting'
#subtitle: ""
author: "SIPA U7514 | Instructor: Harold Stolper"
date: 
urlcolor: blue
output: 
  html_document:
...
We've been talking a lot about the unit of observation. We can think of each observation as a draw of one or more random variable that each follow some underlying (population) distribution. Datasets include observations for individual subjects or entities: surveyed individuals, states, countries, families, schools, years, firms, etc. 
...
We'll talk more about data structure when we cover panel data later in the semester.


### Unit of observation vs unit of analysis

In some instances you'll have to make a distinction between the **unit of observation** across different input data sources, and the **unit of analysis** you ultimately choose to work with. Consider the datasets we used for the subway fare evasion enforcement analysis:
...
* Ultimately we relied on an analysis sample that aggregates the microdata to the subway station level, i.e. the *unit of analysis* is the subway station area.

The decision to collapse the data to aggregate units (subway station areas) alloweds us to use variation that is more useful for exploring racial disparities in enforcement. Instead of using variation in race across arrested individuals, we used variation in racial composition across neighborhoods.


## Obtain the sample size and dimensions of the dataset

How many observations are there, and how many variables (i.e. how many rows and columns)? We've already seen a number of functions to obtain this information, including `str()`, `nrow()`, and `dim()`.

Keep in mind that sometimes the raw data you read in to R might have some rows of data that aren't really observations -- e.g. blank rows, subtotals, or headers. The data cleaning tools we covered in the previous lectures can help you identify and remove/recode uninformative values and observations.


## Explore individual variables and clean/recode/transform

Random variables exhibit variation in our sample that we need to understand and describe. Before starting to think about covariation *between* variables, it's important to describe the variation *within* variables--the distribution of a variable in a sample of data. We've already covered R functions for doing this, but let's review some guiding principles and approaches we should use for different types of variables. 
...
We can visualize the distribution of a continuous variable with a histogram using `ggplot()` and the `geom_histogram()` geometry. We can also compute the mean, median, standard deviation, min, max, and percentiles. We have previously reviewed the `summary()`, `summarize()` and `count()` functions for computing summary statistics. If quantiles or percentiles are of interest, try the `quantiles()` function. 

Here are some examples of R code used to analyze arrests per 100k swipes (a continuous variable) for the grouped subway-station level data we prepared for the end of Assignment 2 and Assignment 4.

```{r, message = FALSE, warning = FALSE}
# load grouped station data
...
Here are some examples of EDA tools for a categorical variable, the uncleaned race input variable from the BDS client microdata for arrested individuals.

```{r, message = FALSE, warning = FALSE}
#load clean BDS microdata
...
ggplot(data = arrests_bds.clean, 
       aes(x = forcats::fct_infreq(race_eth))) + 
...
Whenever you want to account for a variable with categorical information in a regression, you need to include dummy variables for each category (omitting one base category). Many R functions, such as `lm_robust()` and `lm()` are quite intuitive in that if they see any regressors stored as a factor or a logical, they will automatically create dummy variables for each category and exclude the first level. While this is useful, there are two things to keep in mind.

First, you *cannot* include a categorical variable that is stored as a continuous data type (numeric or integer) as a control variable, because the units of the categories have no inherent meaning and would result in misleading comparisons (see [this](https://www.moresteam.com/whitepapers/download/dummy-variables.pdf) link for a very short refresher about using variables in regression analysis). 

Second, you need to be intentional about which category (level) of a categorical (factor) variable to omit as the base category against which comparisons are made. One way to do this is reorder the levels so that the first level is the category you want to omit as the base category. For sample code to reorder levels, please consult [Lecture2.2, Section 6.3](https://hreplots.github.io/U6614/Lectures-gh/Lecture2-gh/Lecture2.2.html).
...
Regardless of whether a variable is continuous or categorical, we want to make sure all of the values that appear for that variable make sense--the coding should be correct, consistent, and useful. In some cases you may need to recode special values as `NA`, recode a column to have a new set of categorical values (e.g. collapsing into fewer categories or creating dummy variables), and/or transform continuous variables (e.g. taking the log of a variable), as covered in Lectures 3.1 and 2.3. 
...
We have previously seen a number of functions for recoding values: `if_else()`, `case_when()`, and `recode()`. We can use these functions to recode specific values as `NA` values. In the following example, there is a messy race variable that includes "0" and "-" values that must be recoded as `NA`, while maintaining the rest of the levels used for the race factor variable.

```{r, message = FALSE, warning = FALSE}
# load clean BDS microdata
arrests_bds.clean  %>% 
  mutate(race_temp = if_else(race %in% c("0", "-"), #tells R to look for "0" and "-" values
...
Outliers are important to take note of, but if the data is accurate and representative then we probably don't want to remove these values. External validity concerns may necessitate including *all* observations to ensure the sample is representative of the underlying population. On the other hand, sometimes data validation reveals extreme values that are likely data entry errors or codes for missing values, and we need to recode these values and document our decisions to manipulate values that appeared in the raw data.
...
We'll often refer to this sort of EDA as **descriptive analysis**. This terminology emphasizes that it likely does not uncover causal relationships, but will help motivate further analysis that attempts to account for confounding factors.
...
* **Inference on difference in proportions between groups**: 
  + `t.test()` allows for robust SEs (unequal variance between groups) but no weights
  + `lm_robust()` allows for weights and robust SEs
  + `datasummary_balance()` is great for automated difference-in-means tables, but only reports the mean difference and SE, not an associated p-value.
...
arrests_bds.clean %>% 
  ggplot(aes(x = race_clean, y = hispanic)) +
  geom_count(na.rm = FALSE)

# test difference in Hispanic share between Blacks and whites
  
  # NOTE: to use t.test() to run a diff-in-means test:
  #   - make sure the grouping var is a logical (not a factor)
  
  # first create logical variables for being Hispanic and being Black
  # and filter out race groups that are not Black or White
  arrests_bds_clean_d <- arrests_bds.clean %>% 
    mutate(hisp_log = as.logical(hispanic == "Hispanic")) %>% 
    mutate(race_black_log = as.logical(race_clean == "Black")) %>% 
    filter(race_clean == "White" | race_clean == "Black")
  
  str(arrests_bds_clean_d$hisp_log)
  str(arrests_bds_clean_d$race_black_log)
  
  # diff in mean Hispanic share by Black vs White arrestees (using dummies) 
  
    # t.test()
    t.test(hisp_log ~ race_black_log, 
           data = arrests_bds_clean_d, 
           var.equal = FALSE)
    
    # lm_robust()
    lm_robust(hisp_log ~ race_black_log, 
           data = arrests_bds_clean_d)
    
    # datasummary_balance()
    # NOTE: if there's a column called weights, weighted stats will be calculated
    datasummary_balance(~ race_black_log, 
                        fmt = 2, 
                        data = subset(arrests_bds_clean_d, 
                                      select = c("hisp_log", 
                                                 "race_black_log")))
...
stations %>% 
  ggplot(aes(x = arrperswipe, colour = as.factor(highpov))) + 
...
* **Fit linear/quadratic models**: 
  ...
  + or estimate separately using `lm_robust()`
...
stations %>% 
  ggplot(aes(x = povrt_all_2016, y = arrperswipe)) + 
  geom_point() + 
  ggtitle('Linear regression fit') + 
  labs(x = 'poverty rate', y = 'arrest rate') + 
  geom_smooth(method = 'lm', 
              formula = y ~ x) #add regression line
...
#4. compute P-value column
  ...
  ols1_a <- lm_robust(arrperswipe ~ nblack, data = stations)
  ols1_b <- lm_robust(swipes2016 ~ nblack, data = stations)
  ols1_c <- lm_robust(arrests_all ~ nblack, data = stations)
  ols1_d <- lm_robust(povrt_all_2016 ~ nblack, data = stations)
  ols1_e <- lm_robust(shareblack ~ nblack, data = stations)

  # get p-values for coefficient of interest in each model
  p_a <- summary(ols1_a)$coefficients[2,4]
  p_b <- summary(ols1_b)$coefficients[2,4]
  p_c <- summary(ols1_c)$coefficients[2,4]
  p_d <- summary(ols1_d)$coefficients[2,4]
  p_e <- summary(ols1_e)$coefficients[2,4]
...
For estimating regressions, `lm()` and `lm_robust()` both accepts weights as an argument:
```{r, message = FALSE, warning = FALSE}
# unweighted and weighted regressions of y on x
ols1_lmr_uw <- lm_robust(y ~ x, 
                        se_type = "HC1", #get robust SEs
                        data = xyw.df)
ols1_lmr_w <- lm_robust(y ~ x, 
                        se_type = "HC1", #get robust SEs
                        weights = w, 
                        data = xyw.df)
summary(ols1_lmr_uw)
