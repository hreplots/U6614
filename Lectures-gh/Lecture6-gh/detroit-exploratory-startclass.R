################################################################################
##
## [ PROJ ] Water shutoffs in Detroit: Community Impact by Race and Income
...
##
## Today: what are the characteristics of communities most affected by this problem?
##    - is median income a stronger predictor than racial composition?  
##
...
library(readstata13)  # imports dta files up through Stata 17
library(tidyverse)
library(lubridate)
library(weights)
...
# NOTE: here "cross-sectional" means 1 obs per tract (shutoffs summed over all years)
...
# for A4: play around w/aesthetics and labels to improve these plots, including:
  # xlab() and ylab()
...
# HINT: use labs(color = "") to label legend for the color aesthetic, for example

# for A4: play around w/aesthetics and labels to improve this plot
  
  
## -----------------------------------------------------------------------------
## 2.0 Time series plots of citywide shutoffs (aggregate over all tracts by month)
## -----------------------------------------------------------------------------
...
# in order to solve this, here is one possible solution using complete()
ym_inc <- tract_ym %>% 
