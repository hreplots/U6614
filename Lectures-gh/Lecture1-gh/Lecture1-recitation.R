...
# remember to comment out or remove install code after you run it once!
...
##
##    c. in a single pipe:
##        - include gender.fac in a new data frame 
##        - call the new data frame access2018.temp1, 
##        - also create factors for caste, income source, state and education,
##        - exclude the columns for HHID and date
...
# 1a. 
  ...
  # NOTE: NEVER include View() in your R Markdown file!
  #       if you do your knit document will print the entire data frame!
...
# 1b.
  # mutate() is a tidyverse function, good practice to initialize with a pipe!
...
# 1c. 
  ...
  # some helpful base R syntax for later: 
  
    # subset the first row of access.temp1
    access.temp1[1,]
    
    # subset the cell in the first row, 5th column (i.e. first obs for age)
    access.temp1[1,5]
...
# 1f.
  ...
  table(access_UP$gender.fac)
  prop.table(table(access_UP$gender.fac))[1]

  
  # let's round to 3 decimals
  # output from prop.table() is just a vector, can pass it into a pipe to round
  ...
  # in your RMD file, you generally can't use pipes with in-line code references
  # so here is base R syntax to round as an in-line code reference
  round(prop.table(table(access_UP$gender.fac)), 3)
  
  # syntax for in-line references
  
  t1_UP <- prop.table(table(access_UP$gender.fac))
  t1_UP1 <- t1_UP[1] # access the first element of this table
 ...
 # `r round(t1_UP[1], 3)`
  ...
  # OPTIONAL advanced exercise (something to try on your own if you have time): 
    ...
    # actually...
    # it's a list with another list as the only element of the list (weird!)
...
# 'Reserved' level, while the rest are assigned to 'General'.
  ...
  round(prop.table(table(access_UP$reserved)), 3)
    

# 2d. 
  summary(access_UP$age)[4]
  
  # `r round(summary(access_UP$age)[4], 2)`
