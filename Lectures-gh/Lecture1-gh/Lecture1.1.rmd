---
title: "[REVIEW] Lecture 1.1 Data Object Type and Structure"
...
A list is just an ordered set of elements stored in a vector. The catch is that the elements can include different types of objects. This can look a bit odd, but can be useful for passing arguments of different object types to a function.
...
Let's take a look a closer look at the structure of this list, and the names we assigned to each element of the list.
```{r}
str(list1)
names(list1)
```

Notice that each element of our list has a name we explicitly gave it when creating the list. If we didn't do this, the `names()` function would return a `NULL` value.
