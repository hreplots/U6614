...
# See https://stringr.tidyverse.org/ for a cheat sheet on stringr functions
...
# See https://lubridate.tidyverse.org/ for a cheat sheet on lubridate functions
...
# Loading the Village Fund data
  ...
  # The amounts are in thousand Rupiah (1,000 Rupiah = 0.059 USD as of March 2026)
...
### FILLING IN MISSING VALUES (using fill() function)
  ...
  # A simple Google search leads us to the following powerful function from the tidyr package (part of the tidyverse)
  # By looking at the documentation, this function can: "fill in missing values with the previous or next value"
  vf_data <- vf_data %>%
  ...
  # It seems like we have 4 rows of information for each regency 
  # (each row represents the allocation for a year)
...
### INSPECTING NUMERICAL DATA AND ASSIGNING NAs AS NEEDEED
  ...
  # Alternatively, we can do the following, which checks the # of NA entries in the dataframe
  sum(is.na(vf_data))
