---
title: "Assignment 6: Difference-in-differences with Staggered Treatments"
...
> <span style="color: #1565c0;"><font size="4">ANSWER: The population-weighted average homicide rate of adopting states was 6.40 homicides per 100,000 population, compared to 5.05 in non-adopting states. This tell us something about *selection into the treatment*: states that adopted Stand Your Ground laws had higher homicide rates to begin with.</font> </span>
...
#### [R Hints:<br><br>1. Reshape from long to wide form panel data using `pivot_wider()`, then you can mutate a new column for $\Delta$hom_rate equal to the difference between the two columns containing endyear values. [Here](https://dcl-wrangle.stanford.edu/pivot-basic.html) is a good tutorial on `pivot_wider()`.<br><br> 2. For the plot you can use `geom_bar()` along with `coord_flip()`, as in the first example [here](https://r-graph-gallery.com/267-reorder-a-variable-in-ggplot2.html).] {-}
...
> <span style="color: #1565c0;"><font size="4">ANSWER: Over the period under study, the distribution of homicide rate changes ranges from -3 to just over 2 homicides per 100,000 population, with more states experiencing declines in the homicide rate. Note that some of the states with the biggest declines in homicide rates adopted Stand Your Grand laws at some point during this period; the question is whether adopting the law slowed the decline in homicide rates, i.e. whether the counterfactual post-treatment decline for these states would have been larger.</font> </span>
...
dataFLtoNever.bytreat <- dataFLtoNever %>% 
  ...
  summarise(hom_rate = round(weighted.mean(hom_rate, popwt), 2),
            l_hom_rate = round(weighted.mean(l_hom_rate, popwt), 2)) 
head(dataFLtoNever.bytreat)

```


## Using the data frame you just created, compare time trends in the log of the homicide rate for Florida vs. [the population-weighted] mean for never-treated states. Try to make the visualization as clear and informative as possible, including a vertical line at 2004 to indicate the year before the treatment took effect in Florida. Some other things to consider: axes labels, group labels for the legend, legend positioning, removing the legend title, and axis labels. {#comp1trends}

#### [R Hint: comparing time trends by group using `geom_line()` is something you did in Assignment 5. Additionally, you can use `geom_vline()` to add a vertical line indicating the year before the treatment first took effect in Florida. You will also need to convert evertreated to a factor with appropriate labels for each 'treatment' group, Florida and never-treated states.] {-}
...
# time series plot
  dataFLtoNever.bytreat %>% 
    ggplot(aes(x = year, y = l_hom_rate,
               ...
               color = evertreated)) + 
    geom_line() + 
    geom_vline(xintercept = 2004, # adds vertical line for the year before law takes effect
               linetype = 'dashed',
               color = 'grey20',
               size = .8) +
    theme_minimal() +
...
> <span style="color: #1565c0;"><font size="4">ANSWER: We observe parallel trends in the log of the homicide rate from 2001 through 2004, just before the treatment takes effect in 2005. Outcomes diverge from 2000 to 2001, but the recent trend is a parallel one.</font> </span>
...
> <span style="color: #1565c0;"><font size="4">ANSWER: The fact that the log of the homicide rate is converging prior to the treatment in 2006 suggests that there are omitted time varying factors that are affecting outcomes in the treatment and comparison groups differently. A violation of parallel trends is a form of omitted variable bias which threatens the internal validity of the estimated treatment effect.<br><br>If this narrowing gap would have persisted into the treatment period in the absence of the treatment, it implies that the counterfactual gap between 2006 adopters and never treated states would have been even smaller. If this were the case, estimated effects would be understated relative to the true effect.</font> </span>
...
> <span style="color: #1565c0;"><font size="4">ANSWER: The coefficient of interest, $\hat{\delta}$, is .0694 with a standard error of 0.0559. </font> </span>


## Interpret the estimated coefficient on $D_{it}$. Assess statistical significance.

> <span style="color: #1565c0;"><font size="4">ANSWER: Adopting a Stand Your Ground law is associated with a 0.0694*100 = 6.94% increase in the homicide rate, controlling for time invariant factors that vary between states, and time varying factors shared by all states. The p-value is 0.220, so the estimated effect is not statistically significant with standard errors clustered at the state level. Interestingly, heteroskedasticity-robust standard errors do permit rejection of the null hypothesis of no effect at the 5% significance level (p-value = 0.437); in other words, clustering by state is indeed a more conservative approach to hypothesis testing.</font> </span>
...
> <span style="color: #1565c0;"><font size="4">ANSWER: Because this law is a time varying factor that affects all states, it is already accounted for by year fixed effects. Thus it would not introduce bias in the TWFE DID estimate that limits internal validity.</font> </span>
...
```{r, reltime}
data_es <- data %>% 
  #create new column measuring time relative to the treatment
  mutate(rel_year = if_else(is.na(treatment_date) == T,
                            -1,
                            year - treatment_date)) %>%
  #bin observations 9, 8, 7 and 6 years ahead of the treatment
  mutate(rel_year = if_else((rel_year == -9 | rel_year == -8 | rel_year ==  -7),
                            -6, rel_year))

# inspect relative time variable
...
## Estimate the event study model. `fixest` includes an interaction helper function `i()`, which we'll use to generate the event study interaction terms. As described above, omit the event year immediately before adoption, which allows us to estimate results relative to that year. Fit the model and produce a standard event study plot, which shows $\hat \beta^{pre}_j$ and $\hat \beta^{post}_j$ in relation to relative time $j$.

```{r, twfe-es}
# estimate model and show output
  twfe_es <- feols(l_hom_rate ~ i(rel_year, evertreated, ref = -1) | sid + year, 
                 data = data_es,
                 cluster = ~sid,
                 weights = data_es$population)
  summary(twfe_es)
...
> <span style="color: #1565c0;"><font size="4">ANSWER: The estimates for periods $j = 1$, $j = 2$, $j = 3$, and $j = 5$ are all statistically significant. We know this because the confidence intervals for these estimates do not intersect the horizontal axis.</font> </span>
...
> <span style="color: #1565c0;"><font size="4">ANSWER: The output reports the total weight and coefficient for the three different groups of 2x2 DID comparisons: (1) 5 group treatment years vs never-treated states; (2) 10 earlier group treatment vs. later group comparisons; and (3) 10 later group treatment vs. earlier group comparison.</font> </span>
...
> <span style="color: #1565c0;"><font size="4">ANSWER: The weighted average of the coefficients for these groups equals the estimated effect from Question 4, 0.0694.</font> </span>
