...
census_api_key("09302fc454429244b79892f6907f782ab08b8e4f")

# initialize new data frame to store ACS data from API requests
MI_acs_tract_10_17 <- data_frame()

for (i in 2010:2017) {
  #example of how for-loops work
  print(i)
  
  if (i==2013) {
    print("I HAVE REACHED 2013")
  }
}
...
# for A5: play around w/aesthetics and labels to improve these plots, including:
# xlab() and ylab()
...
# On rmarkdown: `r round(wtd.cor(tract$medianinc, tract$si_1000, weight = tract$pop)[1,1],2)`
  #for in-line coding
...
ggplot(data = tract, 
  ...
  xlab("Share of Black residents") +
  ylab("Median income") +
  labs(color = "Shutoffs per 1,000 residents")
...
# for A5: play around w/aesthetics and labels to improve this plot
...
ggplot(ym_inc,
  ...
  geom_line() +
  theme_bw() +
  ggtitle("Time series plot of shutoffs by tracts above/median income") +
  xlab("Year") +
  ylab("Shutoffs per 1,000 residents") +
...
ggplot(ym_race, aes(x = date, y=si_1000,
                    color = black75)) +
  geom_line() +
  theme_bw() +
  ggtitle("Time series of plot of shutoffs by tracts above/below 75% Black") +
  xlab("Year") +
  ylab("Shutoffs per 1,000 residents") +
